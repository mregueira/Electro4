...
\[
I_{OB}(L=220\mu H) = \frac{V_o \cdot (1-D) \cdot T_s}{2L} = 42.5mA
\]
...
$$C=\frac{(1-D) \cdot T_{s}^{2}}{8\cdot L\cdot \frac{\Delta V_{o_{max}}}{V_o}}=1.073\mu F$$
...
$$I_{L_{max}}= I_o + \frac{(V_i-V_o)\cdot D_{real}\cdot T_s}{L}=142.5mA$$
$$I_{L_{min}}= I_o - \frac{(V_i-V_o)\cdot D_{real}\cdot T_s}{L}=53.6mA$$
...
El ripple que se obtiene a la salida, considerando una $ESR_C=1\Omega$ es del 165mV.
